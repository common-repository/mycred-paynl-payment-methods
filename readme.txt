=== myCRED Pay.nl Payment Methods ===
Contributors: andypay
Link: http://www.pay.nl
Donate link: https://www.pay.nl/
Tags: paynl, paymentmethods, myCRED, ideal, paypal, creditcard, mybank, sofortbanking, mistercash, bancontact, paysafecard, clickandbuy, giropay, incasso, betaalmethoden, buyCRED
Requires at least: 3.0.1
Stable tag: 1.0.0
Tested up to: 4.0
License: GPLv2
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This plugin adds the pay.nl payment methods to myCRED - buyCRED

== Description ==

*By installing this plugin you'll be able to integrate the payment methods of Pay.nl to myCRED. This will only take a few minutes. If the plugin is successfully installed your customers will then be able to buy credits through the payment options such as iDeal, PayPal, Creditcard and so on.*

Pay.nl offers online payment options for webshops and websites. Pay.nl has different payment option packages available. These packages range from the smallest one (XS) to the largest one (XL). Not sure which package is suitable for you? Start with the try-out package called Pioneer XS! With the Pioneer XS you will be able to use all payment options  except for the options credit card and direct debit. Just fill in the registration form and you'll receive a personal account by email. With this account you have direct access to the Pay.nl Administration Panel where you can add your website(s) and install the payment options that are essential for your website. 

With Pay.nl you can add the following payment options to your website:

* iDeal
* Direct debit
* Credit card (Visa / MasterCard)
* Bancontact / Mister Cash
* Giropay
* Sofortbanking
* Banktransfer EU
* PayPal
* ClickandBuy
* Mybank

For the registration form, visit the registration page (in Dutch): [www.pay.nl/registreren](http://www.pay.nl/registreren)<br>
For more information about the payment rates go to [www.pay.nl/tarieven](htp://www.pay.nl/tarieven)<br>
For more information about the payment solution packages, just visit [www.pay.nl/pakketten](http://www.pay.nl/pakketten)

For any further questions please send an email to support@pay.nl

== Installation ==

This section describes how to install the plugin and get it working.

1. Install the plugin via Plugins -> new plugin
2. Activate the plugin through the 'Plugins' menu in WordPress, the name of this plugin is: myCRED Pay.nl Payment Methods.
3. Activate buyCred if you havent already (under myCRED -> Add-ons)
3. Under myCRED -> Payment Gateways, configure the apitoken and serviceid and check enabled.
4. Your customers can now use Pay.nl as a payment method to buy credits!


== Frequently Asked Questions ==

= How can i get an account for pay.nl? =

You can register [here](http://pay.nl/registreren) (dutch and belgian companies only)

= What payment methods are available through this plugin  =

At the moment the plugin supports the following payment methods:

* Cartebleue
* Click and buy
* Giropay
* iDEAL
* Incasso
* Maestro
* Minitix sms
* Mistercash/Bancontact
* Manual transfer
* Mybank
* Paypal
* Paysafecard
* Postepay
* Visa/Mastercard

= What does it cost? =

The easiest way to get started is bij using a free pioneer account.
With this account there are no monthly fees, you'll only pay transaction costs for the transactions you make.
Check the pioneer tarriffs [here](http://pay.nl/tarieven-pioneer)

If you want to be able to use creditcards, you'll need to have at least a professional account.
For more information check: [pakketten](http://pay.nl/pakketten)

Paid accounts have better tarriffs! see: [tariffs](http://pay.nl/tarieven)

== Screenshots ==

1. The settings screen for the pay.nl plugin
2. Select payment method

== Changelog ==

= 1.0.0 =
* first release

